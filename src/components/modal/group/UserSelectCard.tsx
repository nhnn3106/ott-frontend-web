...
const UserSelectCard: React.FC<UserSelectCardProps & { className?: string }> = ({ user, isSelected, onToggle, className = "" }) => {
  return (
    <div
      onClick={() => onToggle(user.user_id)}
      className={`flex items-center gap-3 p-3 hover:bg-gray-50 cursor-pointer rounded-lg transition-colors ${className}`}
    >
      <div
